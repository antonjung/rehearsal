        ...
        if (!isMyLine) {
          ...
          if (myLineMode === 'silence') {
            if (accuracyEnabled && supported) {
              ...
              resetTranscript()
              const heard = await listen({ expectedText: groupText, silenceMs, estimatedMs: gap, maxPauseMs: settings.maxPauseMs ?? 2000, switchToShortSilenceRef: countdownExpiredRef, onSpeechStart: () => { countdownEndMsRef.current = Date.now() + gap } })
              ...
              if (supported) {
                const _heard = await listen({ silenceMs, estimatedMs: gap, maxPauseMs: settings.maxPauseMs ?? 2000, switchToShortSilenceRef: countdownExpiredRef, onSpeechStart: () => { countdownEndMsRef.current = Date.now() + gap } })
                await delay(600)
