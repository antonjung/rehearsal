...
interface AppState {
  ...
  updateVoiceMap: (voiceMap: Record<string, string>) => void
}
    ...
    (set, get) => ({
      ...
      removeScript: (id) =>
        set((s) => ({
          ...
          selectedScriptId:
            s.selectedScriptId === id ? null : s.selectedScriptId,
          rehearsalSettings:
            s.rehearsalSettings?.scriptId === id
              ? null
              : s.rehearsalSettings,
        })),
      ...
      updateVoiceMap: (voiceMap) => {
        const rs = get().rehearsalSettings
        if (!rs) return
        set({ rehearsalSettings: { ...rs, voiceMap } })
      },
    }),
    {
